...
float CalculateAttenuation(vec3 fragpos, vec3 lightpos){
	float distance = length(abs(lightPos - fragpos)) / 3;
...
{          
    ...
    //vec3 lightDir = fs_in.TBN * length(lightPos - fs_in.FragPos);
	...
	float D = DistributionGGX(normal, abs(H),roughness);
